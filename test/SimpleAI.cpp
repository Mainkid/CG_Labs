...
SimpleAI::SimpleAI(Game* game)
{
	this->game = game;

}

void SimpleAI::Initialize()
{
	vertexBC = nullptr;
	...
	boundingBox2.Center = DirectX::XMFLOAT3(0.975f, 0.0f, 0.0f);
	boundingBox2.Extents = DirectX::XMFLOAT3(0.025f, 0.2f, 0.1f);
}


void SimpleAI::Render()
{
...
void SimpleAI::Update(float deltaSec)
{
	float speed = 1.5f;
	...
	/*if (game->wInput->GetInputDevice()->keys->count(Keys::Up) && data.yOffset < 0.8)
	{
		...
		boundingBox2.Center.y += speed * deltaSec;
	}
...
void SimpleAI::InitializeShaders()
{


}

DirectX::BoundingBox SimpleAI::GetBox()
{
	return boundingBox2;
}
